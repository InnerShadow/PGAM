...
def draw_metrics_plot(train_dic, val_dic, test_dic):
    ...
    metrics_keys = ['loss', 'accuracy', 'precision', 'recall', 'f1', 'kappa', 'mcc', 'roc_auc']
    ...
    for train_metric, val_metric, test_metric, metric_name, key in zip(train_metrics, val_metrics, test_metrics, metrics_names, metrics_keys):
        plt.figure(figsize = (8, 5))
        plt.plot(train_metric[key], label = f'Train {metric_name}', color = 'green', linestyle = '--', alpha = 0.6)
        plt.plot(val_metric[key], label = f'Validation {metric_name}', color = 'blue', linestyle = ':', alpha = 0.8)
        plt.plot(test_metric[key], label = f'Test {metric_name}', color = 'red', linestyle = '-', alpha = 1)
